...
\begin{align*}
	...
	\intertext{%
		Hier wende ich die Formel für die Ableitung von Integralen an:
		\[
			\dod{}x \int_{g(x)}^{h(x)} \dif y \, f(y)
			= \dpd hx(x) f\del{h(x)} - \dpd gx(x) f\del{g(x)}
		\]
	}
	&= x^2 \psi(x) \\
	\hat B \hat A \psi(x)
...
Als Einschränkung muss $\lambda \neq 0$ gelten, damit nicht durch 0 geteilt
wird. Falls $\psi$ eine $L^2$ Funktion sein soll, muss $\lambda < 0$ gelten,
damit die Funktion quadratintegrabel ist.
