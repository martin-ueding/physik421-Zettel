...
\[
	\bracket x = \int_a^b \dif x \, \abs{\psi(x, t)}^2 x
\]

Der Mittelwert des Ortsquadrates ist analog, nur das $x$ durch $x^2$
ausgetauscht wird:
\[
	\bracket{x^2} = \int_a^b \dif x \, \abs{\psi(x, t)}^2 x^2
\]
...
\begin{align*}
	...
	&= \psi^\dagger x^2 \psi - \del{\psi^\dagger x \psi}^2 \\
	&= \abs{\psi}^2 x^2 - \abs\psi^4 x^2 \\
\end{align*}

\begin{align*}
	\braproket{\psi}{\hat p^2}{\psi} - \braproket{\psi}{\hat p}{\psi}^2
	&= -\ii \hbar \psi^\dagger \partial_x \psi' - \hbar^2 \del{\psi^\dagger \psi'}^2 \\
\end{align*}
